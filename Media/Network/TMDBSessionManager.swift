...
class TMDBSessionManager {
    ...
    func fetchSeriesCastS(id: Int, completionHandler: @escaping (CastModel?, CustomError?) -> Void) {
        var url = URLRequest(url: TMDBAPI.cast(id: id).endpoint)
        url.httpMethod = "GET"
        url.addValue(APIKey.tmdb, forHTTPHeaderField: "Authorization")
    ...
    func fetchRecommendedSeries2(id: Int, completionHandler: @escaping (RecommendModel?, CustomError?) -> Void)  {
        var url = URLRequest(url: TMDBAPI.recommend(id: id).endpoint)
        url.httpMethod = "GET"
        url.addValue(APIKey.tmdb, forHTTPHeaderField: "Authorization")
